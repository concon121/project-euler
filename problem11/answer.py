  ...
  for line_index in range(len(L)):
    ...
    for index in range(row_length - 1):
      if index <= row_length -4:
        # get row total
        row_total = int(row[index]) * int(row[index+1]) * int(row[index + 2]) * int(row[index + 3])
        if row_total > largest:
          largest = row_total
        print(f'Row Total: {row_total}')
        # get column total
        if line_index <= len(L) - 4:
          col_total = int(row[index]) * int(L[line_index + 1].split(' ')[index]) * int(L[line_index + 2].split(' ')[index]) * int(L[line_index + 3].split(' ')[index])
          if col_total > largest:
            largest = col_total
        print(f'Column Total: {col_total}')
        # get right diaganol
        if line_index <= len(L) - 4:
          diag_total = int(row[index]) * int(L[line_index + 1].split(' ')[index + 1]) * int(L[line_index + 2].split(' ')[index + 2]) * int(L[line_index + 3].split(' ')[index + 3])
          if diag_total > largest:
            largest = diag_total
        print(f'Diag Total: {diag_total}')
      if index >= 3:
        # get left diaganol
        if line_index <= len(L) - 4:
          diag_total = int(row[index]) * int(L[line_index + 1].split(' ')[index - 1]) * int(L[line_index + 2].split(' ')[index - 2]) * int(L[line_index + 3].split(' ')[index - 3])
